...
def index
  ...
  Dir.glob("#{$path_text}*") do |txt_path|
    index = /#{$path_text}([0-9]+)/.match(txt_path)
    index = index[1].to_i
    text_page = /<!\[CDATA\[(.+)\]\]/.match(File.read(txt_path))[1]
    text_page.gsub!(/([0-9]+)[ft]/, "#{$2}").gsub!(/(?:-|–)\s*([0-9]+)/, "").gsub!(/–/, "-")
    text_page.gsub!(/\s*-\s*/, "-").gsub!(/([0-9]+)\s+([0-9]+)/, "#{$1}#{$2}")
    text_page = text_page.scan(/([^0-9]+,\s*)([0-9,\s]+)/)
    text_page = text_page.map {|match| "#{match[0]}#{match[1]}".gsub(/^[\. -\(\)] */, "")}
    text_page = text_page.join("\n")
    text_pages.push(:index => index, :text => text_page)
  end
